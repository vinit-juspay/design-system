...
export const getRadioInputClassNames = (size: RadioSize, isDisabled: boolean): string => {
  const theme = themeConfig.euler.radio;

  return cn(theme.input.base, theme.sizes[size].input, isDisabled && theme.input.disabled);
};

export const getRadioLabelClassNames = (size: RadioSize, isDisabled: boolean): string => {
  const theme = themeConfig.euler.radio;

  return cn(theme.label.base, theme.sizes[size].label, isDisabled && theme.label.disabled);
};

export const getRadioSubtextClassNames = (size: RadioSize, isDisabled: boolean): string => {
  const theme = themeConfig.euler.radio;

  return cn(theme.subtext, theme.sizes[size].subtext, isDisabled && theme.disabledSubtext);
};
