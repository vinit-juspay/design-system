...
const Alert = forwardRef<HTMLDivElement, AlertProps>(
	({
		...
		return (
			<div
				ref={ref}
				className={getAlertContainerStyles(variant, style)}
			>
				<div className={getAlertHeaderContainerStyles()}>
					...
					{(onClose && actionPlacement === AlertActionPlacement.BOTTOM) && (
						<button
							className={getCloseButtonClassNames()}
							onClick={onClose}
							aria-label="Close"
						>
							<X size={16} />
				...
				<div className={`${getBodyClassNames(actionPlacement)}`}>
					...
					{(primaryAction || (onClose && actionPlacement === AlertActionPlacement.RIGHT)) && <div className={getAlertBodyContainerStyles()}>
						{(primaryAction || secondaryAction) && <div className={getAlertBodyContainerStyles()}>
							{primaryAction && (
								<Button
									key="primary-action"
						...
						{(onClose && actionPlacement === AlertActionPlacement.RIGHT) && <div className={getAlertDividerStyles()}></div>}
