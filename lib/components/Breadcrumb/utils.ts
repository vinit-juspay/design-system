 ...
 */
export const getBreadcrumbItemClassNames = (isActive: boolean): string => {
  const breadcrumbTheme = themeConfig.euler.breadcrumb;

  // Apply default styles to all items
  const baseStyles = [breadcrumbTheme.item.default, breadcrumbTheme.sizes.md];
...
export const getBreadcrumbContainerClassNames = (): string => {
  ...
  return cn(breadcrumbTheme.base.container);
};
...
export const getDividerClassNames = (): string => {
  ...
  return cn(breadcrumbTheme.base.divider);
};
