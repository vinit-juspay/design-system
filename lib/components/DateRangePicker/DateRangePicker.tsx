...
const DateRangePicker = forwardRef<HTMLDivElement, DateRangePickerProps>(({
  ...
  const [startDate, setStartDate] = useState(
    formatDate(selectedRange.startDate, "dd/MM/yyyy")
  );
  ...
  useEffect(() => {
    if (value) {
      ...
      setStartDate(formatDate(value.startDate, "dd/MM/yyyy"));
      setEndDate(formatDate(value.endDate, "dd/MM/yyyy"));
  ...
  const formatDateDisplay = () => {
    ...
    const formattedStart = formatDate(selectedRange.startDate, dateFormat) + 
      (showTimePicker ? `, ${formatDate(selectedRange.startDate, "HH:mm")}` : '');
    
    if (allowSingleDateSelection && selectedRange.startDate.getTime() === selectedRange.endDate.getTime()) {
      return formattedStart;
    }
    
    const formattedEnd = formatDate(selectedRange.endDate, dateFormat) + 
      (showTimePicker ? `, ${formatDate(selectedRange.endDate, "HH:mm")}` : '');
