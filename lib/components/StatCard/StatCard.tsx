...
const StatCard = ({
  ...
  const effectiveVariant =
    (variant === StatCardVariant.LINE || variant === StatCardVariant.BAR) && (!chartData || chartData.length === 0)
  ...
  return (
    <div className={cn("rounded-lg h-[190px] border border-gray-200 overflow-hidden bg-white shadow-xs p-4 space-y-6", className)}>
      {effectiveVariant !== StatCardVariant.NUMBER && <div className="flex flex-col gap-1">
        <div className="flex items-start gap-2">
          {titleIcon && <div className="w-5 h-5 flex items-center justify-center">{titleIcon}</div>}
        ...
        <div className={`flex items-start flex-col ${titleIcon ? "pl-7" : ""}`}>
          <div className="w-full flex items-center gap-1">
            ...
            {formattedChange && (
              <span
                className={cn("ml-2 text-xs", change?.type === ChangeType.INCREASE ? "text-green-600" : "text-red-600")}
              >
                {formattedChange}
              </span>
            )}
          ...
          {effectiveVariant === StatCardVariant.PROGRESS_BAR && progressValue && (
            <div className="w-full h-5 flex items-center gap-4">
              <div className="w-full h-full flex flex-1 rounded-xs overflow-hidden">
                ...
                <div className="bg-white h-full [background-image:repeating-linear-gradient(to_right,_#e1e4ea,_#e1e4ea_5px,_transparent_1px,_transparent)] [background-size:10px_10px]" style={{ width: `${100 - progressValue}%` }}></div>
              </div>
