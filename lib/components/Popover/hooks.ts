...
export const usePopoverState = (controlledOpen?: boolean, onOpenChange?: (open: boolean) => void) => {
...
export const usePopoverPosition = (
    ...
    const updatePosition = useCallback(() => {
        ...
        const boundaryRect = collisionBoundaryRef?.current?.getBoundingClientRect() || new DOMRect(
            0,
            0,
            window.innerWidth,
            window.innerHeight
        ...
        if (primaryPlacement === "top" && availableSpace.top < contentRect.height + offset) {
            finalPlacement = secondaryPlacement ? `bottom-${secondaryPlacement}` as Placement : Placement.BOTTOM;
        } else if (primaryPlacement === "bottom" && availableSpace.bottom < contentRect.height + offset) {
            finalPlacement = secondaryPlacement ? `top-${secondaryPlacement}` as Placement : Placement.TOP;
        } else if (primaryPlacement === "left" && availableSpace.left < contentRect.width + offset) {
            finalPlacement = secondaryPlacement ? `right-${secondaryPlacement}` as Placement : Placement.RIGHT;
        } else if (primaryPlacement === "right" && availableSpace.right < contentRect.width + offset) {
            finalPlacement = secondaryPlacement ? `left-${secondaryPlacement}` as Placement : Placement.LEFT;
        }

        // Calculate position
        let top = 0;
        let left = 0;
        const [finalPrimaryPlacement] = finalPlacement.split("-") as ["top" | "right" | "bottom" | "left", string | undefined];
