...
import { MenuItemProps, MenuItemType, MenuItemState, MenuItemAction } from './types';
import {
...
export const submenuManager = {
  ...
  setActiveSubmenu(id: string | null, parentId: string | null = null) {
    ...
    window.dispatchEvent(
      new CustomEvent('submenu-changed', {
        detail: {
          id,
          parentId,
...
const MenuItem = forwardRef<HTMLDivElement, MenuItemProps>(
  (
    ...
    const uniqueId = useRef<string>(
      id || `menu-item-${Math.random().toString(36).substring(2, 9)}`
    );
    ...
    return (
      <div
        ...
        className={cn(
          itemClassName,
          hoverClass,
          themeConfig.euler.menuv2.menuItem.rounded,
          className
        )}
      ...
      >
        {/* Left slot */}
        {hasSlotL && slotL && <div className={getSlotLClassNames()}>{renderIcon(slotL)}</div>}
        ...
        {hasShortcut && shortcutValue && (
          <div className={getShortcutClassNames()}>{shortcutValue}</div>
        )}
        ...
        {isMultiSelect ? (
          ...
          hasSlotR1 && slotR1 && <div className={getSlotR1ClassNames()}>{renderIcon(slotR1)}</div>
        )}

        {/* Right slot 2 */}
        {hasSlotR2 && slotR2 && <div className={getSlotR2ClassNames()}>{renderIcon(slotR2)}</div>}
        ...
        {hasSubmenu && (
          <div className={getSlotR2ClassNames()}>
            <ChevronRight
              className={cn(
                themeConfig.euler.menuv2.iconSize,
                themeConfig.euler.menuv2.chevronColor
              )}
            />
          </div>
        )}
