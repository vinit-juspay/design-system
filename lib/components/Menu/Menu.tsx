 ...
 **/
const Menu = React.forwardRef<React.ElementRef<typeof DropdownMenu.Content>, MenuProps>(
  (
    {
      children,
      items,
    ...
    const renderMenuItem = (item: MenuItemWithSeparatorProps, index: number) => {
      if ('isSeparator' in item && item.isSeparator) {
        return (
          <DropdownMenu.Separator key={`separator-${index}`} className={getSeparatorClassNames()} />
        );
      }
          ...
          return (
            ...
            >
              <div className={getThreeColumnLayoutClassNames()}>
                {/* Left Slot */}
                {item.leftSlot && (
                  <div className={getLeftSlotClassNames()}>{item.leftSlot.content}</div>
                )}
                {/* Legacy icon support */}
                ...
                <div className={getColumnContentClassNames()}>{item.content}</div>
      ...
      if (standardItem.hasSubmenu && standardItem.submenuItems) {
        return (
          <DropdownMenu.Sub key={`submenu-${index}`}>
            ...
            >
              <div className={getThreeColumnLayoutClassNames()}>
                ...
                {standardItem.leftSlot && (
                  <div className={getLeftSlotClassNames()}>{standardItem.leftSlot.content}</div>
                )}
                {/* Legacy icon support */}
                ...
                <div className={getFlexColumnClassNames()}>
                  <div className={getColumnContentClassNames()}>{standardItem.content}</div>
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.SubContent className={menuClassNames} sideOffset={8} alignOffset={-4}>
        ...
        return (
          ...
          >
            <div className={getThreeColumnLayoutClassNames()}>
              ...
              <div
                className={
                  standardItem.subtext ? getFlexColumnClassNames() : getColumnContentClassNames()
                }
              >
    ...
    return (
      <DropdownMenu.Root {...rootProps} onOpenChange={handleOpenChange}>
        ...
        <DropdownMenu.Portal>
          ...
          >
            ...
            {multiSelect?.enabled ? (
              <ul className="py-1 overflow-y-auto">{filteredItems.map(renderMenuItem)}</ul>
            ) : (
